export function handleDatabaseError(error: unknown): {
  ...
  if (err.code === 'ECONNREFUSED') {
    const isLocalhost = err.hostname?.includes('localhost') || err.hostname?.includes('127.0.0.1');
    if (isLocalhost) {
      errorMessage = 'Cannot connect to localhost from deployed server. Use a publicly accessible database or run the app locally.';
    } else {
      errorMessage = 'Connection refused. Check if the database server is running and accepting connections.';
    }
    ...
    const hostname = err.message?.match(/(?:to |connect to )([^\s:]+)/)?.[1] || '';
    ...
    if (isPrivateIP) {
      ...
      errorMessage = 'Connection timeout. The database may be behind a firewall, not accepting external connections, or the hostname/port may be incorrect.';
    }
  }
